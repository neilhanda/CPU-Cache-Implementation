...
Block size: 4 bytes.
'''
# Reading all the trace files through the function
def myfunc(trace):
	print("--------",trace,"--------")
	#------------------------------------------
	...
	for i in range(0,len(l)):
		...
		if(len(d[l[i][15:30]])==0):
			d[l[i][15:30]].append(l[i][0:15])
			misses=misses+1
			continue
		
		# Otherwise, search in the cache line with index 
		# corresponding to the address.
		for j in range(0,len(d[l[i][15:30]])):
			if(d[l[i][15:30]][j]==l[i][0:15]):
				found=1
				hits=hits+1
				hitvalue=l[i][0:15]
